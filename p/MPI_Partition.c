...
void parseFile(int argc, char ** argv) {
  ...
  while(counter < (masterBoard_rows * masterBoard_columns)) {
    ...
    if(currentChar == 42) {
      masterBoard[counter] = 1; // new box (num_fox, num_rabbit);
      counter++;
    }
...
void calculateBoard() {
      ...
      for(int j = 0; j < 8; j++) {
        if(myNeighborIDs[j] > -1) {
          switch(j) {
            ...
            case 2:
              sendEdge = malloc(sizeof(char) * 1);
              sendEdge[0] = localBoard[2*myCoords.lengthX+2];
              sendSize = 1;
              tag = SW_UPDATE;
              break;
            case 3:
  ...
  if(identity < actualPartitions)
    MPI_Isend(localBoard, (myCoords.lengthX + 2) * (myCoords.lengthY + 2), MPI_CHAR, 0, BOARD_MESSAGE, MPI_COMM_WORLD, &lastRequest);
